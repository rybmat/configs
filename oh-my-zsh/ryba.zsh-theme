...
PROMPT='%{$ROOT_ICON_COLOR%}$ROOT_ICON%{$reset_color%}%{$MACHINE_NAME_COLOR%}%m:%{$reset_color%} %{$PROMPT_SUCCESS_COLOR%}%c:%{$reset_color%} %{$GIT_PROMPT_INFO%}$(git_prompt_info)%{$GIT_DIRTY_COLOR%}$(git_prompt_status)%{$reset_color%}  %{$PROMPT_PROMPT%}%n%{$GIT_PROMPT_INFO%}:%{$reset_color%} '

RPROMPT='⌚ %{$GIT_CLEAN_COLOR%}%*%{$reset_color%}'
